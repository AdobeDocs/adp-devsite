...
async function fetchProfileAvatar(userId) {
  try {
    ...
    if (req) {
      ...
      const avatarUrl =
        res?.user?.images?.["138"] ?? "/hlx_statics/icons/avatar.svg";
  ...
  window.imsLoaded =
    ...
    new Promise((resolve, reject) => {
      ...
      if (isHlxPath(window.location.host)) {
        ...
        !isHlxPath(window.location.host) &&
        isStageEnvironment(window.location.host)
      ) {
...
async function loadLazy(doc) {
  ...
  if (getMetadata("template") === "documentation") {
    ...
    const headings = main.querySelectorAll(
      "h2:not(.side-nav h2):not(footer h2), h3:not(.side-nav h3):not(footer h3)"
    );
    if (
      !document.querySelector(".hero, .herosimple") &&
      headings.length !== 0
    ) {
